...
| 팀별 비밀번호 (1~10팀) | `server/index.ts` 의 `TEAM_PASSWORDS` 상수 참조 (행사 종료된 값이라 운영 의미는 없음) |
...
본 프로젝트에서 Docker는 **Server 전용**입니다. Client 는 Render 의 Static Site
호스팅으로 배포되므로 별도 컨테이너가 필요하지 않습니다.
...
CI 의 `docker-verify` 잡은 매 push 마다 server 이미지를 빌드한 뒤 컨테이너를
실행해 `/health` 응답을 확인한다. Client 의 빌드 정합성은 별도의 client 잡
(lint / typecheck / build)에서 검증된다.
...
| `docker-verify` | Buildx로 server Dockerfile 빌드 → 서버 컨테이너 실행 → `/health` curl 로 응답 확인 |
