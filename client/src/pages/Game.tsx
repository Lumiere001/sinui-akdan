...
export function Game() {
  ...
  // GPS 권한 확인 — 위치가 들어오면 granted
  useEffect(() => {
    if (position) setGpsGranted(true)
  }, [position])
  ...
  if (!gpsGranted) {
    return (
      <div style={{
        ...
        {gpsError && (
          <div style={{
            background: colors.errorBg || 'rgba(239,68,68,0.1)', border: `1px solid ${colors.error}30`,
            borderRadius: radius.lg, padding: spacing.md, marginBottom: spacing.lg,
            fontSize: typography.sm, color: colors.error, textAlign: 'center', maxWidth: 320,
          }}>
            {gpsError}
          </div>
        )}
        <div style={{
          ...
          <div>Android: 주소창 왼쪽 자물쇠 &gt; 위치 &gt; 허용</div>
        </div>
        <button
